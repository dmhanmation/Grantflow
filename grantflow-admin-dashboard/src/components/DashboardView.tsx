      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3.5">
        ...
        <div className={`p-4 rounded-xl border shadow-xs transition ${
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-amber-800 uppercase tracking-wider block">Doc Expiry (30d)</span>
            {expiringDocs30Days.length > 0 && <span className="w-2 h-2 rounded-full bg-amber-600 animate-pulse" />}
          </div>
          <span className="text-2xl font-black text-amber-900 mt-1 block">{expiringDocs30Days.length}</span>
          <span className="text-[10px] text-amber-800/80 mt-0.5 block">
            {totalDocsCount === 0 ? '0 docs uploaded' : expiringDocs30Days.length > 0 ? 'Expiring in Library' : `${totalDocsCount} docs active`}
          </span>
        </div>
        ...
        {/* In Preparation */}
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <span className="text-[11px] font-bold text-indigo-600 uppercase tracking-wider block">In Preparation</span>
          <span className="text-2xl font-black text-indigo-700 mt-1 block">{preparingCount}</span>
          <span className="text-[10px] text-slate-500 mt-0.5 block">Drafting & Review</span>
        </div>

        {/* Awaiting Decision */}
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider block">Awaiting Decision</span>
          <span className="text-2xl font-black text-slate-800 mt-1 block">{awaitingDecisionCount}</span>
          <span className="text-[10px] text-slate-500 mt-0.5 block">Submitted to donor</span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[10px] text-rose-700/80 mt-0.5 block">Not selected</span>
        </div>
      </div>
