...
app.post('/api/invitations/create', (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.post('/api/workspaces', (req: AuthenticatedRequest, res: Response) => {
  const orgId = req.organizationId || req.body.organizationId;
  if (!orgId) return res.status(400).json({ error: 'Organization ID is required.' });
...
app.put('/api/workspaces/:id', (req: AuthenticatedRequest, res: Response) => {
  const orgId = req.organizationId || req.body.organizationId;
  if (!orgId) return res.status(400).json({ error: 'Organization ID is required.' });
  const { id } = req.params;
  const ws: OpportunityWorkspace = req.body;
...
app.post('/api/extract-org-profile', async (req: Request, res: Response) => {
  try {
    ...
    const instruction = `You are reading an official institutional document (for example a certificate of incorporation, a CAC registration certificate, a constitution, or an organisational profile) that belongs to a non-profit, NGO, or civil society organisation.
Extract ONLY what the document actually states. Never invent, infer, or guess a value that is not clearly present.
For any text field the document does not clearly contain, return exactly "Not found in document". For lists, return an empty array when nothing is stated.
Be concise and use the exact wording from the document where possible.${documentName ? `\nThe uploaded file is named: ${documentName}.` : ''}`;
