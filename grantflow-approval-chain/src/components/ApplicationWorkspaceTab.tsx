...
export const ApplicationWorkspaceTab: React.FC<ApplicationWorkspaceTabProps> = ({
  ...
  // Resolve active approval chain for this workspace
  const activeChain = resolveChain(orgProfile.approvalChain, workspace.approvalChain);

  // Map each chain stage to the person assigned to it in this workspace
  const stagePersonMap = {
    DepartmentHead: workspace.reviewer || '',
    ProposalLead: workspace.proposalLead || '',
    Reviewer: workspace.intermediateReviewer || '',
    FinalApprover: workspace.finalApprover || orgProfile.defaultFinalApproverName || ''
  };
  ...
  const handleApplyReviewDecision = () => {
    ...
    const updatedSections = sections.map((sec) => {
      if (sec.id !== feedbackModalSecId) return sec;

      if (feedbackAction === 'approve') {
        // Advance through the chain, auto-skipping same-person stages
        const pendingStage = currentPendingStage(sec.reviewStatus, activeChain);
        const approver = pendingStage ? stagePersonMap[pendingStage] : undefined;
        const { newStatus, autoSkipped } = advanceAfterApproval(
          sec.reviewStatus,
          activeChain,
          stagePersonMap,
          approver
        ...
        return {
          ...
          status: isApprovedForSubmission(newStatus) ? ('Complete' as const) : ('Under Review' as const),
          reviewedBy: approver || 'Reviewer',
          reviewedAt: new Date().toISOString(),
          reviewerNotes: feedbackNote || (
            autoSkipped.length > 0
              ? `Approved. Auto-advanced past ${autoSkipped.join(', ')} (same person).`
              : 'Approved.'
          )
        };
      } else {
        return {
          ...sec,
          reviewStatus: 'Returned for Revision' as any,
          status: 'Drafting' as const,
          reviewedAt: new Date().toISOString(),
          reviewerNotes: feedbackNote || 'Returned for revision.'
        };
      }
    });
  ...
  const handleSubmitToHoD = (sectionId: string) => {
    const firstStatus = firstSubmittedStatus(activeChain);
    const updatedSections = sections.map((sec) => {
      if (sec.id === sectionId) {
        return {
          ...sec,
          reviewStatus: firstStatus as any,
  ...
  const narrativeMetrics = useMemo(() => {
    ...
    const approved = sections.filter(
      (s) => isApprovedForSubmission(s.reviewStatus)
    ).length;
                        ...
                        <div className="text-right">
                          <span
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border ${
                              isApprovedForSubmission(sec.reviewStatus)
