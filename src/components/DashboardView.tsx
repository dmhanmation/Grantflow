...
import { AdminSetupDashboard } from './AdminSetupDashboard';
import {
  Sparkles,
  Clock,
  Calendar,
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Every non-admin operational user is rendered through ONE role-aware dashboard component.
  // This is the single-identity invariant: the same logged-in person must never be rendered
  // simultaneously in legacy assignment, department, executive, or role-perspective panels.
  const operationalRoles = ['Officer', 'DepartmentHead', 'ProposalLead', 'Reviewer', 'FinalApprover'];
  const hasOperationalDashboard = Boolean(
    !isAdmin &&
    currentUser &&
    (operationalRoles.includes(currentUser.role) || currentUser.roles?.some(role => operationalRoles.includes(role)))
  );
  ...
  // ---- ADMIN SHORT-CIRCUIT: Admins see only the setup dashboard ----
  if (isAdmin) {
    return (
      <AdminSetupDashboard
        orgProfile={orgProfile!}
  ...
  // ---- OPERATIONAL STAFF SHORT-CIRCUIT ----
  // PersonalizedRoleDashboard owns the single visible identity header and switches between
  // only the roles actually assigned to the user. No duplicate name/profile blocks can render.
  if (hasOperationalDashboard && currentUser && orgProfile) {
    return (
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-xl px-5 py-4 md:px-6 md:py-5 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
