...
const formatUserRoleLabel = (role?: string) => {
  ...
  return labels[role] || role.replace(/([a-z])([A-Z])/g, '$1 $2');
};
...
export default function App() {
  ...
  const [invitationToken, setInvitationToken] = useState<string | null>(() => {
    try {
      ...
      if (window.location.pathname === '/join') return params.get('token');
  ...
  // Cache-buster: Force-clear legacy browser localStorage on startup
  ...
  try {
    ...
    if (activeVersion !== CLEAN_SLATE_VERSION) {
      // Clear out all grantflow-related localStorage keys
      const keysToRemove: string[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith('grantflow_')) {
          keysToRemove.push(k);
        }
      }
      keysToRemove.forEach(k => localStorage.removeItem(k));
      localStorage.setItem('grantflow_clean_slate_version', CLEAN_SLATE_VERSION);
      localStorage.setItem('grantflow_current_user', JSON.stringify(defaultAdminUser));
      localStorage.setItem('grantflow_org_profile', JSON.stringify(cleanSlateOrgProfile));
    }
  } catch (e) {
  ...
  // Email deep links: open the exact workspace/task/profile area referenced by a notification.
  useEffect(() => {
    if (invitationToken) return;
    try {
      const params = new URLSearchParams(window.location.search);
      const profileTab = params.get('profile');
      if (profileTab) {
        setTargetProfileTab(profileTab);
      ...
      if (!workspaceId || opportunities.length === 0) return;
      const exists = opportunities.some(ws => ws.id === workspaceId);
      if (!exists) return;
      setSelectedWorkspaceId(workspaceId);
      setTargetWorkspaceTab(params.get('tab') || 'overview');
      setTargetTaskId(params.get('task') || undefined);
      setActiveTab('workspaces');
    } catch {
      // Ignore malformed deep-link query strings and leave normal navigation unchanged.
    }
  }, [opportunities, invitationToken]);
  ...
  if (invitationToken) {
    return (
      <InvitationAcceptScreen
        ...
        onAccepted={(user, org, token) => {
          handleAuthSuccess(user, org, token);
                ...
                >
                  ...
                  <div className="hidden lg:block">
                    <span className="text-xs font-bold text-slate-800 block truncate max-w-[130px] leading-tight">
                      {currentUser.jobTitle || formatUserRoleLabel(currentUser.role)}
                    </span>
                  </div>
